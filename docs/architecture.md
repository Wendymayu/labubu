...
| 层次 | 技术 | 说明 |
|------|------|------|
| **后端语言** | Go 1.19 | 标准库为主，无 ORM |
| **数据存储** | chDB (ClickHouse 嵌入式) / 内存存储 | CGO 构建用 chDB，纯 Go 构建用 memStore，统一 `Store` 接口 |
| **指标存储** | tstorage | 嵌入式 TSDB，Prometheus 兼容查询 |
| **日志存储** | chDB / 内存存储 | 与 Trace 共用 Store 接口 |
...
- **构建标签系统**：`cgo` / `local_engine` 选择 chDB vs memStore；`dev` 选择磁盘读取前端 vs 嵌入
...
```
      ...
      ▼        ▼        ▼
┌─────────┐ ┌─────────┐ ┌─────────┐
│ chDB    │ │memStore │ │ config  │
│Store    │ │Store    │ │加载器   │
│(CGO)    │ │(纯Go)   │ │         │
└────┬────┘ └────┬────┘ └────┬────┘
     │           │           │
     ▼           ▼           ▼
 ClickHouse   内存切片   labubu.yaml
 嵌入式DB    + maps     + pricing.yaml
 +SQL查询    +JSON持久  (配置+默认定价)
```

#### 技术要点

- **双实现策略**：CGO 构建用 chDB（生产级 SQL 查询），纯 Go 构建用 memStore（开发/跨平台）
- **统一接口**：所有 Handler 仅依赖 `Store` 接口，不直接访问 chDB
- **chDB Store**：线程安全（`sync.Mutex`），Schema 从 `schema.sql` 初始化，SQL 由 `chdb_query.go` 生成
- **memStore**：`sync.RWMutex` 保护，LLMConfig/Pricing/Diagnosis 持久化到 `data/memstore.json`
- **成本计算触发**：`InsertSpans` 后异步调用 `UpdateTraceCost` goroutine（chDB Store），memStore 内联计算
- **会话分组**：Trace 含 `jiuwenclaw.session.id` 属性时自动归入 Session
- **数据清理**：`Purge` 定期删除过期 Trace（默认 24h / 10000 条），配置驱动
...
| **双实现切换** | 构建标签选择 chDB (生产) / memStore (开发)，同一接口 | `chdb.go` / `memstore.go` |
